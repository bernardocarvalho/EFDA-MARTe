...
int is_cpu_stall(int cpuid){
	return rt_get_task_state(&rt_smp_linux_task[cpuid]);
}
EXPORT_SYMBOL(is_cpu_stall);

void rt_schedule_linux(RT_TASK *rt_task, int cpuid) {
	switch_rtai_tasks(rt_task, &rt_smp_linux_task[cpuid], cpuid);
	rt_schedule();
	switch_rtai_tasks(&rt_smp_linux_task[cpuid], rt_task, cpuid);
	rt_schedule();
}
EXPORT_SYMBOL(rt_schedule_linux);
...
EXPORT_SYMBOL(give_back_to_linux);
